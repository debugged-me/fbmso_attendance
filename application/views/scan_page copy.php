...
<style>
  .scan-wrap{position:relative;width:100%;max-width:680px;margin:auto;}
  #reader{position:relative;width:100%;height:auto;min-height:320px;background:#000;border-radius:12px;overflow:hidden}
  #scanStatus{
    position:absolute;bottom:12px;left:50%;transform:translateX(-50%);
    background:rgba(17,24,39,.7);color:#fff;border:1px solid rgba(255,255,255,.15);
    padding:6px 12px;border-radius:999px;font-size:.85rem;backdrop-filter:blur(4px);
    z-index:4;
  }
  #scanTips{margin-top:8px;font-size:.85rem;color:#475569}
  .tip{display:inline-flex;align-items:center;border:1px solid #e2e8f0;border-radius:999px;padding:4px 10px;margin:3px 6px 0 0;background:#f8fafc}
  ...
  #profileModal #pBadge { border-radius:999px; padding:4px 10px; font-weight:700; text-transform:uppercase; letter-spacing:.06em; font-size:.75rem; color:#fff; }
  #profileModal .pcard-when { font-size:.85rem; color:#6b7280; }
  #profileModal .pcard-main { display:flex; gap:14px; }
  #profileModal .pro-avatar { width:84px; height:84px; border-radius:12px; overflow:hidden; background:#f3f4f6; border:1px solid #e5e7eb; display:flex; align-items:center; justify-content:center; }
  ...
  .alert-primary{background:#e5efff;border-color:#bfd2ff;color:#1e40af}
</style>
...
(function(){
  ...
  function setStrip(color){ if (pStatusStrip) pStatusStrip.style.background = color; }
  function setBadge(text, cls){ pBadge.className = 'badge ' + (cls || 'badge-light'); pBadge.textContent = text; }
  function timeNow(){ try { return moment().format('MMM D, YYYY HH:mm:ss'); } catch(e){ return ''; } }
  function setStatus(text, cls){ statusEl.textContent = text; statusEl.className = ''; if (cls) statusEl.classList.add(cls); }
  function addLine(text, cls){ const row = document.createElement('div'); row.textContent = text; if (cls) row.className = cls; logEl.prepend(row); }
  function clearLog(){ logEl.innerHTML = '<div class="text-muted">Waiting for scans…</div>'; }
  ...
  function applyPhoto(imgEl, iconEl, url){
    ...
    if (url){
      ...
      imgEl.onerror = () => { imgEl.style.display='none';  iconEl.style.display='flex'; };
      imgEl.src = url;
    } else {
      imgEl.style.display='none';
      iconEl.style.display='flex';
  ...
  async function getProfile(student_number){
    let sn = student_number, name = null, course = null, major = null, photo_url = null;
    try{
    ...
    return { sn, name, course, major, photo_url };
  }
  ...
  function defaultConfig(){
    ...
    return {
      ...
      videoConstraints: {
        facingMode: { ideal: 'environment' },
        width:  { ideal: desktopBoost ? 2560 : 1920 },
        height: { ideal: desktopBoost ? 1440 : 1080 },
  ...
  async function start(cameraDeviceId){
    ...
    try {
      ...
      if (vid) {
        ...
        vid.muted = true;
        try { vid.play && vid.play(); } catch(_e){}
      }
  ...
  function parsePayload(str){
    let token = null, activity = String(<?= (int)$activity->activity_id ?>);
  ...
  function onScanSuccess(decodedText){
    ...
    .then(async (r) => {
      ...
      catch (_e) { throw new Error('HTTP '+r.status+' '+r.statusText+' — '+txt.slice(0,240)); }
  ...
  fileInput.addEventListener('change', function(e){
    ...
    scanImageFile(file).then(()=> { fileInput.value = ''; });
  });
  ...
  // isn't triggered by a tap, which caused "NotAllowedError: Permission
  // denied" when the camera was accessed on page load.
  ...
  // (Optional) face hints
  let faceInterval = null, lastFaceAt = 0;
